...
def _validate_channel_indices(
    ...
    return np.asarray(channel_indices, dtype=np.int64)


class MeanChannelPacketizer:
    """Average selected PCM16 channels and emit fixed-size mono packets."""
    ...
    def __init__(
        ...
        *,
        ...
        self.channel_indices = _validate_channel_indices(
            source_channels,
            channel_indices,
        )
    ...
    def push_pcm16(self, payload: bytes) -> list[bytes]:
        ...
        mono = source[:, self.channel_indices].mean(axis=1).astype(np.int16)
        self._pending_output = np.concatenate((self._pending_output, mono))
